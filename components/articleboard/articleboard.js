...
export default function Board({ page, folder }) {
  ...
  let [selectedBlock , setSelection] = useState()
  ...
  const updateSelection = async() => {
    if(window.getSelection() != undefined && (window.getSelection()).type == 'Caret' )
    {
      console.log("inside select")
      await setSelection([window.getSelection().focusNode , window.getSelection().focusOffset])
    }
  }
  ...
  const PopupWindow = ({ popupState }) => {
    if (popupState.newImagePopup) { return (<NewImagePopup keyName="newImagePopup" topic={folder} togglePopup={handlePopupClick} />) }
    if (popupState.openImagePopup) { return (<OpenImagePopup keyName="openImagePopup" topic={folder} togglePopup={handlePopupClick} addImage={actImage} />) }
    else { return null; }
  }
  ...
  return (
    <div className="p-3" style={{ width: '85%', height: '100%' }}>
      ...
      <div className={`${styles.kpEditorCard}`}>
        ...
        <div className={`${styles.kpEditorEditingPassage}`} id="kp-editor" contentEditable suppressContentEditableWarning={true} onKeyUp={countWords} onMouseUp={updateSelection}>
          Type something
        </div>
